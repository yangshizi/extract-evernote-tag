...
def main():
    ...
    for note in new_notes.notes:
        ...
        if not note.attributes.sourceURL and not note.attributes.sourceApplication and not note.attributes.source:
            continue
        print "标签(tags):%s" %(",".join(note_store.getNoteTagNames(note.guid)))
        print '-'*120
